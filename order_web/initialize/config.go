...
func init() {
	...
	configFileName := fmt.Sprintf("order_web/%s_pro.yaml", configFilePrefix)
	if debug {
		configFileName = fmt.Sprintf("order_web/%s_prev.yaml", configFilePrefix)
	}
	...
	cc := constant.ClientConfig{
		...
		LogDir:              "./order_web/consul/log",
		CacheDir:            "./order_web/consul/cache",
		LogLevel:            "debug",
	}
	// 至少一个ServerConfig
